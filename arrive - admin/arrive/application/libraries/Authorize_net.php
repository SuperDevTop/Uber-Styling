...
class Authorize_net
{
    ...
    private $api_login_id = '98Pa7n9AG';			// API Login ID
    private $api_transaction_key = '7Ka32nbL46U4f929';	// API Transaction Key
		...
		elseif( is_string($response) && strpos($response, '|') !== FALSE )
		{
			...
			if( isset($res[0]) )
			{
				switch( $res[0] )
				{
					...
					$this->card_number = isset($res[50]) ? $res[50] : '';
					$this->card_type = isset($res[51]) ? $res[51] : '';
